...
struct FText: View {
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 0.0, content: {
            Text("\(self.fraction.whole == 0 && self.fraction.original < 0 ? "-" : "")\((self.fraction.whole != 0) || (self.fraction.whole == 0 && self.fraction.parts.d == 1) ? self.fraction.whole.description : "")\(self.fraction.isFraction ? " " : "")").font(.headline)
