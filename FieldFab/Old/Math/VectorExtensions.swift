...
extension SCNVector3 {
    ...
    func translate<T: BinaryFloatingPoint>(x: T) -> SCNVector3 { SCNVector3(self.x + Float(x), self.y, self.z) }
    func translate<T: BinaryFloatingPoint>(y: T) -> SCNVector3 { SCNVector3(self.x, self.y + Float(y), self.z) }
    func translate<T: BinaryFloatingPoint>(z: T) -> SCNVector3 { SCNVector3(self.x, self.y, self.z + Float(z)) }
    ...
    func normalized() -> SCNVector3 {
        let len = self.length()
        if len > 0 { return self.divideScalar(scale: len) } else { return SCNVector3(0.0, 0.0, 0.0) }
    }
